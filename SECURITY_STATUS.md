...
網頁回覆更新：Access Token 現在經 AES-256-GCM 加密保存於後端，不再是用完即丟。Secret 與 Token 使用不同的附加驗證資料；都不回傳前端。新回覆路由只允許擁有者向既有對話傳送，透過交易式 outbox、租約、每 OA 節流及 LINE Retry Key 防止重複發送，並在 23 小時後拒絕不明結果的重試。36 項單元測試及 62 項官方規則模擬通過；本機瀏覽器測試僅模擬發送，沒有傳送真實 LINE 訊息。下方「Token 不保存」屬先前收件版本的紀錄。
